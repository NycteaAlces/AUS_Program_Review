...
output$DefForecast <- renderPlot(ggplot (data = OL()$DefCnt) + geom_point(aes(x=Year, y=DefCnt), colour = "red", alpha=I(0.5) )+geom_text(aes(label=DefCnt))+geom_line(data=OL()$DefCnt, aes(x=Year,y=DefCnt))+ylim(0, max(OL()$DefCnt$DefCnt))+labs(title=paste("Number of survey units that are in deficit based on the proposed funding amounts of $",input$AEPBudget, "000 from AEP, and $", input$EMSDBudget, "000 from EMSD."), y="Number of survey units that are in deficit", x="Forecasted year"))
