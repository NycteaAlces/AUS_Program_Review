...
shinyServer(function(input, output,session) {#----
  ...
  OL <- eventReactive(list(input$AEPBudget, input$EMSDBudget, input$ForecastYr, input$TopUp), { ####----
    ...
    if (input$TopUp==T){
      for(y in (1+as.integer(format(Sys.Date(), "%Y"))):ForecastYr){
    ...
    colnames(Schedule.V2) <- c("Region", "WMU", as.numeric(input$ForecastYr):(1+as.integer(format(Sys.Date(), "%Y"))))
    Schedule.V3 <- Schedule.V2[,c(1,2, ncol(Schedule.V2):3)]
    colnames(Deficit.V1) <- c("Region", "WMU", as.numeric(input$ForecastYr):(1+as.integer(format(Sys.Date(), "%Y"))))
  ...
  output$DefForecast <- renderPlot(ggplot () + geom_point(data = OL()$DefCnt,aes(x=Year, y=DefCnt),colour = "red", alpha=I(0.5) )+geom_text(data = OL()$DefCnt,aes(x=Year, y=DefCnt,label=DefCnt), hjust=0, vjust=0)+geom_line(data=OL()$DefCnt, aes(x=Year,y=DefCnt))+ylim(0, max(OL()$DefCnt$DefCnt))+labs(title=paste("Number of survey units that are in deficit based on the proposed funding amounts of $",input$AEPBudget, "000 from AEP, and $", input$EMSDBudget, "000 from EMSD."), y="Number of survey units that are in deficit", x="Forecasted year"))
